...
def orange_init_api():
    ...
    app.add_url_rule('/account/user/alias', view_func=acc_user_alias, methods=['POST', 'get'])

    # 这里是系统用户接口
    app.add_url_rule('/server/sys/user/list', view_func=sys_user_list, methods=['POST', 'get'])
    app.add_url_rule('/server/sys/user/name_list', view_func=sys_name_list, methods=['POST', 'get'])
    ...
    app.add_url_rule('/local/cron/com_list', view_func=local_cron_com_list, methods=['POST', 'get'])
